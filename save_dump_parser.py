...
# Sections to capture as raw text for Tier 2 display
RAW_CAPTURE_SECTIONS = [
    "SKILL MASTERY TIERS",
    "ATTRIBUTE BONUS PREVIEW (Level-Up)",
    ...
    "ENCHANTED ITEMS CATALOG",
    "ITEM CONDITION REPORT",
    "ENCHANTMENT CHARGE STATUS",
    "MOST VALUABLE ITEMS (Top 10)",
    "HEAVIEST ITEMS (Top 10)",
    ...
    "SIGIL STONES",
    "LIGHT SOURCES",
    "POTION EFFECTS SUMMARY",
    "POTION EFFECTIVENESS SUMMARY",
    "WORLD STATE SUMMARY",
    "MAIN QUEST PROGRESS",
    "EXPLORATION ACHIEVEMENTS",
    "SPELL SCHOOL ANALYSIS",
    "SPELL TYPE BREAKDOWN",
    "SPELL RANGE ANALYSIS",
    "MAGICKA EFFICIENCY",
    "MAGIC EFFECT COVERAGE",
    "ALCHEMY CROSS-REFERENCE",
    "ALCHEMY APPARATUS",
    "INGREDIENT STOCK ANALYSIS",
...
class SaveDumpParser:
    ...
    def parse(self) -> CharacterData:
        ...
        data.skills, data.skills_current = self._parse_skills()
        ...
        data.skill_xp_progress, data.level_up_progress = self._parse_skill_xp_progress()
        data.training_progress = self._parse_training_progress()
        data.quick_keys = self._parse_quick_keys()
        data.status_effects = self._parse_status_effects()
        data.actor_value_modifiers = self._parse_actor_value_modifiers()
        data.item_conditions = self._parse_item_conditions()
        data.enchantment_charges = self._parse_enchantment_charges()
        data.stolen_items = self._parse_stolen_items()
        data.gold_economy = self._parse_gold_economy()

        # Enhanced character info (racial abilities, class details, equipped items)
        data.racial_abilities, data.class_details, data.equipped_items_detail = (
            self._parse_character_info_enhanced()
        )

        # Tier 2 bulk raw capture
        data.raw_sections = self._capture_raw_sections()
    ...
    def _parse_character_info(self) -> CharacterInfo:
        ...
        # Try to extract name from header if PLAYER CHARACTER section is missing
        if info.name == "Hero of Kvatch":
            for line in self.lines[:15]:
                line = line.strip()
                if line.startswith("Character:"):
                    # Format: "Character: Zodiark, Level 21 Dark Elf Mage"
                    raw = line.split(":", 1)[1].strip()
                    # Name is before the first comma
                    if "," in raw:
                        info.name = raw.split(",")[0].strip()
                    break
        ...
        for line in self._get_section_lines("CHARACTER INFO"):
            ...
            if line.startswith("Level:"):
                ...
                # Format: "Race: Dark Elf (LOC_FN_DarkElf) (0x000191C1)"
                raw = line.split(":", 1)[1].strip()
    ...
    def _parse_skills(self) -> Tuple[Dict[str, int], Dict[str, int]]:
        """Parse SKILLS section. Returns (base_skills, current_skills)."""
        skills = {}
        skills_current = {}
    ...
    def _parse_spells(self) -> List[Spell]:
        """Parse SPELLS section including spell effects.

        Handles two formats:
        Base game: "[0] Name (editor_id) (0xFORM) Type: Spell, Cost: 24"
                   "       Effect Name (editor_id) [CODE] (Range) Mag: X Dur: Ys [AV]"

        Player-created: "[0] Name (editor_id) (0xFORM)"
                        "       Type: Spell, Cost: Auto, Mastery: Novice"
                        "       Effects: N"
                        "         [0] Effect Name (editor_id) - Mag: X, Dur: Y, Area: Z, Range: Self, Cost: C"
        """
        spells = []
        in_player_created = False
        ...
        for line in self._get_section_lines("SPELLS"):
            ...
            if stripped.startswith(("Number of Spells:", "Base Game Spells:",
                                   "Player-Created Spells:", "By Type:", "By School:")):
                continue

            # Section separator
            if stripped.startswith("--- Player-Created Spells"):
                in_player_created = True
                continue
            if stripped.startswith("--- Base Game Spells"):
                in_player_created = False
                continue

            # Spell header: "[0] Name (editor_id) (0xFORM) Type: Spell, Cost: 24"
            # or player-created: "[0] Name (editor_id) (0xFORM)"
            spell_header = re.match(
                r'\[\d+\]\s+(.+?)\s+\((0x[0-9A-Fa-f]+)\)\s*(?:Type:\s*(\w+),\s*Cost:\s*(\d+))?',
                stripped
            )
            if spell_header:
                spell_name = spell_header.group(1)
                spell_form_id = spell_header.group(2)
                spell_type = spell_header.group(3) or ""
                cost = 0
                if spell_header.group(4):
                    try:
                        cost = int(spell_header.group(4))
                        if cost > 100000:
                            cost = 0
                    except ValueError:
                        pass
                ...
                spells.append(Spell(
                    ...
                    is_player_created=in_player_created
                ))
                continue
            ...
            # Player-created: "Type: Spell, Cost: Auto, Mastery: Novice"
            type_m = re.match(r'Type:\s*(\w+),\s*Cost:\s*(\w+)', stripped)
            if type_m:
                current_spell.spell_type = type_m.group(1)
                continue

            # Skip flags/effects count lines
            if stripped.startswith(("Flags:", "Effects:", "EffectCode:")):
                continue
            ...
            if base_effect:
                effect_name = base_effect.group(1).strip()
                effect_code = base_effect.group(2)
                effect_range = base_effect.group(3)
                params = base_effect.group(4).strip()

                mag = 0
                dur = 0
                area = 0
                mag_m = re.search(r'Mag:\s*(\d+)', params)
                dur_m = re.search(r'Dur:\s*(\d+)', params)
                if mag_m:
                    mag = int(mag_m.group(1))
                if dur_m:
                    dur = int(dur_m.group(1))

                current_spell.effects.append(SpellEffect(
                    name=effect_name,
                    magnitude=mag,
                    duration=dur,
                    area=area,
            ...
            # Player-created effect line:
            # "[0] Restore Health (LOC_FN_REHE) - Mag: 100, Dur: 2, Area: 0, Range: Self, Cost: 726.16"
            player_effect = re.match(
                r'\[\d+\]\s+(.+?)\s+\([^)]*\)\s*-\s*Mag:\s*(\d+),\s*Dur:\s*(\d+),\s*Area:\s*(\d+),\s*Range:\s*(\w+)',
                stripped
            )
            if player_effect:
                effect_name = player_effect.group(1).strip()
                mag = int(player_effect.group(2))
                dur = int(player_effect.group(3))
                area = int(player_effect.group(4))
                effect_range = player_effect.group(5)

                # Try to extract effect code from a nearby EffectCode line — handled via next iteration
                current_spell.effects.append(SpellEffect(
                    name=effect_name,
                    magnitude=mag,
                    duration=dur,
    ...
    def _parse_skill_xp_progress(self) -> Tuple[List[SkillXPProgress], str]:
        """Parse SKILL EXPERIENCE PROGRESS section."""
        skills = []
        level_up_progress = ""

        for line in self._get_section_lines("SKILL EXPERIENCE PROGRESS"):
            line_stripped = line.strip()

            # Level-up progress line
            if line_stripped.startswith("Level-up progress:"):
                level_up_progress = line_stripped.split(":", 1)[1].strip()
                continue

            # Skill XP line: "    Armorer: 23.14 / 35.92 (64%) {gained 485.0 this lvl}"
            m = re.match(
                r'(\w[\w\s]*):\s*([\d.]+)\s*/\s*([\d.]+)\s*\((\d+)%\)(?:\s*\[.*?\])?\s*(?:\{gained\s*([\d.]+)\s*this\s*lvl\})?',
                line_stripped
            )
            if m:
                gained = float(m.group(5)) if m.group(5) else 0
                skills.append(SkillXPProgress(
                    skill_name=m.group(1).strip(),
                    current_xp=float(m.group(2)),
                    needed_xp=float(m.group(3)),
                    percentage=int(m.group(4)),
                    gained_this_level=gained
                ))

        return skills, level_up_progress

    def _parse_training_progress(self) -> TrainingProgress:
        """Parse TRAINING PROGRESS section."""
        tp = TrainingProgress()
        for line in self._get_section_lines("TRAINING PROGRESS"):
            line = line.strip()
            m = re.match(r'Training sessions this level:\s*(\d+)\s*/\s*(\d+)', line)
            if m:
                tp.sessions_used = int(m.group(1))
                tp.sessions_max = int(m.group(2))
        return tp

    def _parse_quick_keys(self) -> List[QuickKeySlot]:
        """Parse QUICK KEYS section."""
        slots = []
        for line in self._get_section_lines("QUICK KEYS"):
            line = line.strip()
            if not line or line.startswith("Total assigned"):
                continue
            # "Slot 1: Rejuvenate (UI_UserInputText_10_84_17) (0xFF002478) [Spell]"
            m = re.match(
                r'Slot\s+(\d+):\s+(.+?)\s+\([^)]*\)\s+\((0x[0-9A-Fa-f]+)\)\s+\[(\w+)\]',
                line
            )
            if m:
                slots.append(QuickKeySlot(
                    slot_number=int(m.group(1)),
                    name=m.group(2),
                    form_id=m.group(3),
                    slot_type=m.group(4)
                ))
            elif line.startswith("Slot") and "(empty)" in line:
                slot_m = re.match(r'Slot\s+(\d+):', line)
                if slot_m:
                    slots.append(QuickKeySlot(
                        slot_number=int(slot_m.group(1)),
                        name="(empty)",
                        form_id="",
                        slot_type=""
                    ))
    ...
    def _parse_status_effects(self) -> StatusEffects:
        """Parse STATUS EFFECTS & ADDITIONAL ACTOR VALUES section."""
        se = StatusEffects()
        field_map = {
            ...
            "NightEyeBonus": "nighteye_bonus",
            "AttackBonus": "attack_bonus",
            "DefendBonus": "defend_bonus",
            "CastingPenalty": "casting_penalty",
            "Blindness": "blindness",
            "Paralysis": "paralysis",
            "Silence": "silence",
            "Confusion": "confusion",
            "DetectItemRange": "detect_item_range",
            "SwimSpeedMultiplier": "swim_speed_multiplier",
            "WaterBreathing": "water_breathing",
            "WaterWalking": "water_walking",
            "DetectLifeRange": "detect_life_range",
            "Telekinesis": "telekinesis",
            "Vampirism": "vampirism",
            "Darkness": "darkness",
            "ResistWaterDamage": "resist_water_damage",
        }

        for line in self._get_section_lines("STATUS EFFECTS & ADDITIONAL ACTOR VALUES"):
            line = line.strip()
            if not line or line.endswith(":"):
                continue
            # "  Aggression: 0 (Base: 0)" or "  NightEyeBonus: 0.0 (Base: 0.0)"
            m = re.match(r'(\w+):\s*([\d.\-]+)', line)
            if m:
                name = m.group(1)
                if name in field_map:
                    value = float(m.group(2))
                    attr_name = field_map[name]
                    if attr_name in ("aggression", "confidence", "energy", "responsibility"):
                        setattr(se, attr_name, int(value))
                    else:
                        setattr(se, attr_name, value)

        return se

    def _parse_actor_value_modifiers(self) -> List[ActorValueModifier]:
        """Parse ACTOR VALUE MODIFIERS section."""
        modifiers = []
        seen_direct = False
        for line in self._get_section_lines("ACTOR VALUE MODIFIERS"):
            line = line.strip()
            if not line or line.startswith("Format:") or line.startswith("Total non-zero:"):
                continue
            # Stop at the "Direct Memory Read" duplicate section
            if line.startswith("Direct Memory Read"):
                seen_direct = True
                continue
            if seen_direct:
                continue
            # "  Agility                  [    -5.0 |     +0.0 |     +0.0]"
            m = re.match(
                r'(\w[\w\s]*?)\s+\[\s*([\d.\-+]+)\s*\|\s*([\d.\-+]+)\s*\|\s*([\d.\-+]+)\s*\]',
                line
            )
            if m:
                modifiers.append(ActorValueModifier(
                    actor_value=m.group(1).strip(),
                    fortify_max=float(m.group(2)),
                    script_modifier=float(m.group(3)),
                    damage_modifier=float(m.group(4))
                ))
        return modifiers

    def _parse_item_conditions(self) -> List[ItemCondition]:
        """Parse ITEM CONDITION REPORT section."""
        conditions = []
        for line in self._get_section_lines("ITEM CONDITION REPORT"):
            line = line.strip()
            if not line:
                continue
            # "  Daedric Helmet: 1024 / 1350 (76%)"
            m = re.match(r'(.+?):\s*(\d+)\s*/\s*(\d+)\s*\((\d+)%\)', line)
            if m:
                conditions.append(ItemCondition(
                    name=m.group(1).strip(),
                    current_hp=int(m.group(2)),
                    max_hp=int(m.group(3)),
                    percentage=int(m.group(4))
                ))
        return conditions

    def _parse_enchantment_charges(self) -> List[EnchantmentCharge]:
        """Parse ENCHANTMENT CHARGE STATUS section."""
        charges = []
        for line in self._get_section_lines("ENCHANTMENT CHARGE STATUS"):
            line = line.strip()
            if not line or "enchanted weapons" in line:
                continue
            # "  Retaliation of Spirit: 14 / 110 (13%) [LOW]"
            m = re.match(r'(.+?):\s*(\d+)\s*/\s*(\d+)\s*\((\d+)%\)\s*\[(\w+)\]', line)
            if m:
                charges.append(EnchantmentCharge(
                    name=m.group(1).strip(),
                    current_charge=int(m.group(2)),
                    max_charge=int(m.group(3)),
                    percentage=int(m.group(4)),
                    status=m.group(5)
                ))
        return charges

    def _parse_stolen_items(self) -> List[StolenItem]:
        """Parse STOLEN ITEMS CHECK section."""
        stolen = []
        for line in self._get_section_lines("STOLEN ITEMS CHECK"):
            line = line.strip()
            if not line or line.startswith("Total:"):
                continue
            # "  Iron Claymore x1 - Owner: Savlian Matius (0x00028007)"
            m = re.match(r'(.+?)\s+x(\d+)\s*-\s*Owner:\s*(.+?)\s*\((0x[0-9A-Fa-f]+)\)', line)
            if m:
                stolen.append(StolenItem(
                    name=m.group(1).strip(),
                    quantity=int(m.group(2)),
                    owner_name=m.group(3).strip(),
                    owner_form_id=m.group(4)
                ))
        return stolen

    def _parse_gold_economy(self) -> GoldEconomy:
        """Parse GOLD ECONOMY section."""
        ge = GoldEconomy()
        for line in self._get_section_lines("GOLD ECONOMY"):
            line = line.strip()
            if not line:
                continue
            if line.startswith("Current Gold:"):
                try:
                    ge.current_gold = int(line.split(":", 1)[1].strip())
                except ValueError:
                    pass
            elif line.startswith("Stores Invested:"):
                try:
                    ge.stores_invested = int(line.split(":", 1)[1].strip())
                except ValueError:
                    pass
            elif line.startswith("Houses Owned:"):
                try:
                    ge.houses_owned = int(line.split(":", 1)[1].strip())
                except ValueError:
                    pass
            elif line.startswith("Horses Owned:"):
                try:
                    ge.horses_owned = int(line.split(":", 1)[1].strip())
                except ValueError:
                    pass
            elif line.startswith("Largest Bounty:"):
                raw = line.split(":", 1)[1].strip()
                m = re.match(r'(\d+)', raw)
                if m:
                    ge.largest_bounty = int(m.group(1))
        return ge

    def _parse_character_info_enhanced(self) -> Tuple[List[str], str, str]:
        """Extract racial abilities, class details, and equipped items from CHARACTER INFO.

        The section has this structure:
            Level: 21
            Race: Dark Elf (LOC_FN_DarkElf) (0x000191C1)
              Racial Abilities:
                [0] Ancestor Guardian ...
                [1] Dark Elf Fire Resistance ...
            Class: Mage (LOC_FN_Mage) (0x000105D6)
              Specialization: Magic
              Favored Attributes: Willpower, Intelligence
              Major Skills: ...
            Birthsign: The Atronach ...
              Powers/Spells:
                [0] Atronach Birthsign ...
                     Spell Absorption ...
            Active Spell: ...
            --- Equipped Items ---
              [Armor] Daedric Helmet ...
        """
        racial_lines = []
        class_lines = []
        equipped_lines = []

        section_lines = self._get_section_lines("CHARACTER INFO")

        # State machine with named zones
        zone = None  # None, 'racial', 'class', 'birthsign', 'equipped'
        ...
        for line in section_lines:
            ...
            # Detect zone transitions based on actual dump format
            if stripped.startswith("Racial Abilities:"):
                zone = 'racial'
                continue
            elif stripped.startswith("Class:"):
                # "Class: Mage ..." — capture this line plus indented children
                zone = 'class'
                class_lines.append(stripped)
                continue
            elif stripped.startswith("Birthsign:"):
                zone = 'birthsign'
                # Include birthsign header + its powers
                class_lines.append("")  # separator
                class_lines.append(stripped)
                continue
            elif stripped.startswith("--- Equipped Items ---"):
                zone = 'equipped'
                continue
            elif stripped.startswith("Active Spell:"):
                # Standalone line, not part of any sub-block
                class_lines.append("")
                class_lines.append(stripped)
                zone = None
                continue
            elif stripped.startswith("Level:") or stripped.startswith("Race:"):
                # Top-level lines already parsed elsewhere
                if stripped.startswith("Race:"):
                    # Include race line in class_lines for context
                    class_lines.append(stripped)
                zone = None
                continue
            elif stripped.startswith("Total Armor Rating:"):
                equipped_lines.append(stripped)
                zone = None
                continue

            # Collect into current zone
            if zone == 'racial':
                racial_lines.append(stripped)
            elif zone in ('class', 'birthsign'):
                class_lines.append(stripped)
            elif zone == 'equipped':
                equipped_lines.append(stripped)
    ...
    def _capture_raw_sections(self) -> Dict[str, str]:
        """Capture all Tier 2 sections as raw text strings."""
        raw = {}
        for section_name in RAW_CAPTURE_SECTIONS:
            lines = self._get_section_lines(section_name)
            if lines:
                raw[section_name] = "\n".join(lines)
        return raw
